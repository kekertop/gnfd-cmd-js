...
class CrosschainService {
    ...
    public async transferOut(
        ...
        try {
            ...
            console.log(`Successfully transferred out ${amountFlag} BNB to ${toAddressFlag}. Transaction hash: ${response.transactionHash}`);
        } catch (ex) {
    ...
    public async mirror(
        ...
        try {
            ...
            console.log(`Successfully mirrored ${resourceFlag} with ID ${idFlag}. Transaction hash: ${response.transactionHash}`);
        } catch (ex) {
            throw new Error('Unable to transfer out');
        }
    }

    private async mirrorInternal(client: Client, resourceType: string, props: MsgMirrorObject | MsgMirrorGroup | MsgMirrorBucket): Promise<TxResponse> {
        switch (resourceType) {
            case 'object':
